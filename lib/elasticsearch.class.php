...
class ProudElasticSearch
{
	...
	public function __construct()
	{
		...
		elseif ($this->agent_type === 'subsite') {
	...
	public function em_save_events($post_id, $post, $update)
	{
		...
		// Skip autosaves (classic + editor autosave revisions).
		...
		// If you *only* want to skip heartbeat (not all AJAX):
		if (wp_doing_ajax() && ! empty($_POST['action']) && $_POST['action'] === 'heartbeat') {
			return;
		}
		...
		foreach ($events_array as $event) {
			if ((int) $recurring->event_id !== (int) $event->recurrence_id) {
				continue;
			}

			$post_indexable->index((int) $event->post_id, true);
		}
	}
